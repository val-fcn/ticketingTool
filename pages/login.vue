<template>
    <UContainer class="min-h-screen flex items-center justify-center">  
        <UCard v-if="authMode === 'login'" class="auth-form w-80 mt-[-10vh]">
          ...
          <UForm @submit="handleLogin" :state="state" :validate="validate" class="space-y-4">
          ...
          <div class="mt-4 text-center">
            <a href="#" @click.prevent="authMode = 'forgot'" class="text-primary-500 hover:underline">Mot de passe oublié ?</a>
          </div>
        </UCard>
        ...
        <UCard v-if="authMode === 'forgot'" class="auth-form w-80 mt-[-10vh]">
          ...
          <UForm @submit="handleForgotPassword" :state="state" class="space-y-4">
            ...
            <UButton type="submit" block>Envoyer le lien de réinitialisation</UButton>
          </UForm>
          <div class="mt-4 text-center">
            <a href="#" @click.prevent="authMode = 'login'" class="text-primary-500 hover:underline">Retour à la connexion</a>
          </div>
        </UCard>
    </UContainer>
</template>
